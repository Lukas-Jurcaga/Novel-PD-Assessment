...
data_root_dir = '../../Study/Data/features/'


def get_all_dataset_paths(dataset_type):
    all_names = os.listdir(data_root_dir + dataset_type + '/')
    all_dataset_paths = []
    for name in all_names:
        all_dataset_paths.append(data_root_dir + dataset_type + '/' + name)
...
def get_null_count_columns(data_frame):
    ...
    for column in data_frame.columns:
        ...
        if curr_null_count_acc > 0:
            null_counts[column] = [curr_null_count_acc]
    return null_counts
