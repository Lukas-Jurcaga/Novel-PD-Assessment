function TemporalFeatures = calculate_TemporalGaitFeatures_McCamley_BAM(Settings,AccVT,Fs,PlotSettings)
%function TemporalFeatures = calculate_TemporalGaitFeatures_McCamley_BAM(Settings,AccVT,Fs,PlotSettings)
%
% OUTPUTS
% * TemporalFeatures.
...
%       - StepDuration = duration of step cycles, defined between initial contacts (non consecutive, i.e. from the same foot)
%       - StrideDuration = duration of stride cycles, defined between two initial contacts (non consecutive)
...
%       
% HISTORY
% 2019/2nd/September functionized - Encarna Micó Amigo
%
% REMARKS
...
for iIC = 1:size(InitialContacts,2)-1                                       % loop for peak finding between Initial Contact events 
    ...
    [~,FinalContacts_ShiftedIC] = findpeaks(DerivativeSig2_Detrended(IC_1st:IC_2nd),'MinPeakHeight',ThresholdPeak_Amplitude_FC);
    ...
    if ~isempty(FinalContacts_ShiftedIC)                                    % if no peak with a threshold was found
        FinalContacts(iIC) = IC_1st + FinalContacts_ShiftedIC(1)-1;
    end
end
...
if size(IC,2) > Settings.MinNumEvents                                       % at least, for a gait bout counting on xxx steps (number predefined in settings)
    ...
    % 4.4 Swing durations                                                   % the rest of the stride cycle which is not stance
