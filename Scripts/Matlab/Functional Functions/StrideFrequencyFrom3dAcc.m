function [StrideFrequency, QualityInd, ModalFrequencies] = StrideFrequencyFrom3dAcc(AccXYZ, F)
...
%% History
%  March 2014, version 1.0, adapted from StrideFrequencyFrom3dAccBosbaan
...
if numel(F) == 1, % Calculate the PSD from time series AccXYZ, F is the sample frequency
    AccFilt = detrend(AccXYZ);  % Detrend data to get rid of DC component in most of the specific windows
    LenPSD = 10*F;
    for i=1:3,
